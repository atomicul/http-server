...
namespace http_uploader::configuration {
    ...
    unsigned int Configuration::port() {
        return std::stoi(getEnv("PORT"));
    }

    std::string Configuration::getEnv(const char *name) {
        const char *value = std::getenv(name);
        if (value == nullptr)
            throw std::invalid_argument(std::string{} + "Environment variable not set: " + name);

        return value;
    }

    std::string Configuration::formatAsDirectory(std::string_view path) {
        bool match = std::regex_match(path.begin(), path.end(), std::regex(R"regex(^(.+)\/([^\/]+)$)regex"));
